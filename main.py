...
from chatXYZ import run, run_test, victim
import logging
...
def index():
    if request.method == "POST":
        ...
        try:
            if api_key_mode == "user":
                ...
                if api_key == "":  # API key is not provided in user mode; show error
                    ...
                    result = run_test(query, api_key=api_key, victim="MarieCurie")
        ...
        return render_template("index-%s.html"%(victim), result=result, query=query, show_api_key_box=SHOW_API_KEY_BOX)
    else:
        return render_template("index-%s.html"%(victim), show_api_key_box=SHOW_API_KEY_BOX)
